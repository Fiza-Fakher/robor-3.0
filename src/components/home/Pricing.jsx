...
<button className='flex justify-center items-center gap-2 sm:mr-6 w-full sm:w-auto
  ...
  Get Started Now <FaArrowRightLong size={18} className="sm:hidden" /><FaArrowRightLong size={22} className="hidden sm:inline" />
</button>
</div>
{/* Pricing Cards */}
       <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:justify-center mt-8 sm:mt-10 items-stretch gap-6 px-0 sm:px-6">
        {priceCards.map((item, index) => (
          <div
            key={index}
            className="bg-[var(--primary)] w-full lg:w-84 h-auto lg:h-[34rem] rounded-2xl flex flex-col justify-between p-6 sm:p-8 transition-all duration-300 relative"
          >
            {/* Toggle Button */}
